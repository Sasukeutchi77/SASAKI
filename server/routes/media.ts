...
function getCloudinaryFolder(usageType: MediaUsageType): string {
  switch (usageType) {
    case 'avatar':
      return 'fasoinfo/avatars';
    case 'cover':
      return 'fasoinfo/covers';
    case 'article_cover':
      return 'fasoinfo/articles';
    case 'article_gallery':
      return 'fasoinfo/galleries';
    case 'article_video':
      return 'fasoinfo/videos';
    case 'press_card':
      return 'fasoinfo/press_cards';
    case 'media_logo':
      return 'fasoinfo/media_logos';
    default:
      return 'fasoinfo/uploads';
  }
}
...
mediaRouter.post('/signature', requireAuth, mediaUploadLimiter, (req: AuthenticatedRequest, res: Response) => {
  ...
  const { folder = 'fasoinfo/uploads', usageType = 'general' } = req.body;
  const user = req.user!;
