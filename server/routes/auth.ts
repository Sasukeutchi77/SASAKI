...
authRouter.post('/register', authRateLimiter, (req, res) => {
  ...
  const cleanBio = bio ? sanitizeText(bio, { maxLength: 500 }) : 'Lecteur sur FasoInfo';
...
authRouter.post('/login', authRateLimiter, (req, res) => {
  ...
  if (user.status === 'suspended') {
    return res.status(403).json({
      error: 'Votre compte a été suspendu par l’équipe de modération de FasoInfo. Veuillez contacter le support.',
