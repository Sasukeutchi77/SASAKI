...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loginWithGoogle = async () => {
    ...
    try {
      if (isFirebaseActive && firebaseAuth) {
        ...
        // Fallback for demo when Firebase Client credentials are not yet injected
        const demoEmail = 'google.user@fasoinfo.bf';
        const res = await api.login({ email: demoEmail, password: 'user123' }).catch(() => {
          return api.register({
            name: 'Citoyen FasoInfo (Google)',
            email: demoEmail,
            password: 'google_oauth_pass_safe',
            accountType: 'user',
          });
        });
        setToken(res.token);
  ...
  const resetPassword = async (email: string) => {
    if (isFirebaseActive && firebaseAuth) {
      ...
      // Simulate confirmation for demo mode
