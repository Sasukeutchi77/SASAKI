...
export const AdminDashboardModal: React.FC<AdminDashboardModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 md:p-6 bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div
        className="w-full max-w-7xl h-[94vh] bg-stone-50 rounded-2xl md:rounded-3xl shadow-2xl flex flex-col border border-stone-200 overflow-hidden"
        role="dialog"
        aria-modal="true"
      >
        {/* Flash Notification Toast */}
        {flash && (
          <div className="absolute top-4 right-4 z-50 px-4 py-2.5 bg-gray-900 text-white text-xs font-semibold rounded-xl shadow-lg border border-gray-700 flex items-center gap-2 animate-slideDown">
        ...
        <div className="px-5 py-3.5 bg-white border-b border-stone-200 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="hidden sm:inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-red-100 text-red-700">
                  Accès Restreint
                </span>
              </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-2 rounded-xl bg-stone-100 text-stone-700 hover:bg-stone-200 hover:text-stone-900 transition flex items-center gap-1.5 text-xs font-medium"
            >
        ...
        {/* Navigation Tabs Bar */}
        <div className="bg-white border-b border-stone-200 px-4 flex items-center gap-1 overflow-x-auto shrink-0 scrollbar-none">
          <button
            onClick={() => setActiveTab('overview')}
            className={`py-3 px-3.5 text-xs font-bold whitespace-nowrap border-b-2 transition flex items-center gap-2 ${
              activeTab === 'overview'
          ...
          >
            ...
            Utilisateurs
            <span className="ml-1 px-1.5 py-0.5 rounded-full text-[10px] bg-gray-100 text-gray-600 font-semibold">
              {usersList.length}
            </span>
          </button>
          ...
          >
            ...
            {pendingVerifsCount > 0 && (
              <span className="ml-1 px-1.5 py-0.5 rounded-full text-[10px] bg-amber-100 text-amber-800 font-bold animate-pulse">
                {pendingVerifsCount}
              </span>
            )}
          </button>
          ...
          >
            ...
            {pendingReportsCount > 0 && (
              <span className="ml-1 px-1.5 py-0.5 rounded-full text-[10px] bg-red-100 text-red-700 font-bold animate-pulse">
                {pendingReportsCount}
              </span>
            )}
          </button>
